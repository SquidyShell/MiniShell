...
/*   Updated: 2024/03/21 18:41:44 by cviegas          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	needs_to_be_expanded(t_vars *v)
{
	if (v->in_expanded_var && v->index >= v->end_of_var)
		v->in_expanded_var = 0;
	if (v->tokens)
		return (!v->in_expanded_var && v->line[v->index] == '$' && !v->in_quote
		...
		return (!v->in_expanded_var && v->line[v->index] == '$'
			&& v->line[v->index + 1] && !v->in_quote
			&& !is_whitespace(v->line[v->index + 1])
			&& (is_snakecase_or_qmark(v->line[v->index + 1])
				|| is_quote_after_and_not_in_quote(v)));
}
